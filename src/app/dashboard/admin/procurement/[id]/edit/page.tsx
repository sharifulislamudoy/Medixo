...
export default function EditProcurementPage() {
  ...
  const fetchProcurement = async () => {
    try {
      ...
      if (res.ok) {
        ...
        // Extract unique brands from items for filter dropdown
        extractBrandsFromItems(mappedItems);
      } else {
  ...
  const extractBrandsFromItems = (itemList: ProcurementItem[]) => {
  ...
  // Filter items based on search and brand
  useEffect(() => {
    let filtered = items;
    if (searchTerm.trim()) {
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6 p-4 md:p-6">
      ...
      <div className="flex justify-end gap-4">
        ...
        <button
          ...
          disabled={saving}
          className="px-6 py-2 bg-[#0F9D8F] text-white rounded-lg hover:bg-[#0c7d72] disabled:opacity-50 flex items-center gap-2 transition-colors"
        >
          <Save size={18} />
